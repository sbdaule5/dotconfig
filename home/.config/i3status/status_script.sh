...
function music {
  ...
  text=$(mpc status -f "[[%artist%] - [%title%]]"| awk '{if (NR==1) head=$0; if(NR==2){if($1 == "[playing]"){print head " " $3}}}' | sed -e 's/\"//g')
  local json='{ "full_text": "'$text'", "color": "#2787e7" }'
  json_array=$(update_holder holder__music "$json")
}
